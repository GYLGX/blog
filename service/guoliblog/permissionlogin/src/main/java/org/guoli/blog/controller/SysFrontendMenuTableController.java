package org.guoli.blog.controller;



import com.baomidou.mybatisplus.extension.api.ApiController;
import org.guoli.blog.entity.SysFrontendMenuTable;
import org.guoli.blog.service.SysFrontendMenuTableService;
import org.guoli.blog.utlis.R;
import org.springframework.web.bind.annotation.*;

import javax.annotation.Resource;
import java.io.Serializable;
import java.util.List;

/**
 * (SysFrontendMenuTable)表控制层
 * 该类由EasyCode工具生成
 * @since 2020-03-07 13:42:36
 */
@RestController
@RequestMapping("sysFrontendMenuTable")
public class SysFrontendMenuTableController extends ApiController {
    /**
     * 服务对象
     */
    @Resource
    private SysFrontendMenuTableService sysFrontendMenuTableService;

    /**
     * 分页查询所有数据
     *
     * @return 所有数据
     */
    @GetMapping
    public R selectAll() {
        return R.ok().data("list",this.sysFrontendMenuTableService.getAllMenuList());
    }

    /**
     * 通过主键查询单条数据
     *
     * @param id 主键
     * @return 单条数据
     */
    @GetMapping("{id}")
    public R selectOne(@PathVariable Serializable id) {
        return R.ok().data("info",this.sysFrontendMenuTableService.getById(id));
    }

    /**
     * 新增数据
     *
     * @param sysFrontendMenuTable 实体对象
     * @return 新增结果
     */
    @PostMapping
    public R insert(@RequestBody SysFrontendMenuTable sysFrontendMenuTable) {
        if(this.sysFrontendMenuTableService.save(sysFrontendMenuTable)) {
            return R.ok().message("添加新菜单成功");
        }else {
            return R.error().message("添加新菜单失败");
        }
    }

    /**
     * 修改数据
     *
     * @param sysFrontendMenuTable 实体对象
     * @return 修改结果
     */
    @PutMapping
    public R update(@RequestBody SysFrontendMenuTable sysFrontendMenuTable) {
        if(this.sysFrontendMenuTableService.updateById(sysFrontendMenuTable)) {
            return R.ok().message("修改菜单信息成功");
        }else {
            return R.error().message("修改菜单信息失败");
        }
    }

    /**
     * 删除数据
     *
     * @param idList 主键结合
     * @return 删除结果
     */
    @DeleteMapping
    public R delete(@RequestParam("idList") List<String> idList) {
        if(this.sysFrontendMenuTableService.removeByIds(idList)) {
            return R.ok().message("删除菜单成功");
        }else {
            return R.error().message("删除菜单失败");
        }
    }
}